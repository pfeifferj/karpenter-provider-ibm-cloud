...
var (
	// IBM Cloud resource ID pattern
	ibmResourceIDPattern = regexp.MustCompile(`^r[0-9]{3}-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$`)
	// API server endpoint pattern
	apiServerEndpointPattern = regexp.MustCompile(`^https?://[^/]+:\d+$`)
)
...
func (nc *IBMNodeClass) validateNodeClass(nodeClass *IBMNodeClass) (admission.Warnings, error) {
	...
	if nodeClass.Spec.APIServerEndpoint == "" {
		...
		errs = append(errs, fmt.Sprintf("apiServerEndpoint '%s' is not a valid URL (expected format: https://IP:PORT)", nodeClass.Spec.APIServerEndpoint))
	}
	...
	if len(nodeClass.Spec.SecurityGroups) == 0 {
		...
		for _, sg := range nodeClass.Spec.SecurityGroups {
			if !ibmResourceIDPattern.MatchString(sg) {
				if !strings.HasPrefix(sg, "r") || len(sg) < 45 {
					*errs = append(*errs, fmt.Sprintf("security group '%s' appears to be a name, not an ID. Security groups must be specified as IBM Cloud resource IDs (format: r###-########-####-####-####-############)", sg))
				} else {
					*errs = append(*errs, fmt.Sprintf("security group '%s' is not a valid IBM Cloud resource ID", sg))
				}
			}
		}
	}
...
func (nc *IBMNodeClass) validateSubnet(nodeClass *IBMNodeClass) []string {
	...
	if nodeClass.Spec.Subnet != "" {
		subnetPattern := regexp.MustCompile(`^[0-9a-f]{4}-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$`)
		if !subnetPattern.MatchString(nodeClass.Spec.Subnet) {
