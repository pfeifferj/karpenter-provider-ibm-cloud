...
func TestE2EStartupTaints(t *testing.T) {
	...
	// Create deployment that forces node creation with high resource requirements
	// This avoids the chicken-and-egg problem by not using nodeSelector
...
func TestE2ETaintsBasicScheduling(t *testing.T) {
	...
	intolerantDeployment := createResourceIntensiveWorkload(intolerantDeploymentName, testName+"intolerant", nil)
...
func TestE2EUnregisteredTaintHandling(t *testing.T) {
	...
	// Create deployment to force node creation
	deployment := createResourceIntensiveWorkload(deploymentName, testName, nil)
...
func createResourceIntensiveWorkload(name, testLabel string, tolerations []corev1.Toleration) *appsv1.Deployment {
					...
					Containers: []corev1.Container{
						{
							...
							Resources: corev1.ResourceRequirements{
								Requests: corev1.ResourceList{
									...
									// Use very high resource requests to ensure no existing node can satisfy this
